...
class GetNowPlayingMoviePagingUseCase @Inject constructor(private val movieApi: MovieApi) {

    operator fun invoke(): Flow<PagingData<MovieModel>> = Pager(
        config = PagingConfig(20),
        pagingSourceFactory = { CategoryMoviePagingDataSource { movieApi.getNowPlayingMovies(it) } }
    )
        .flow
