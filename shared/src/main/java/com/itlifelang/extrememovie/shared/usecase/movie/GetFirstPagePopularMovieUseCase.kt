...
class GetFirstPagePopularMovieUseCase @Inject constructor(private val movieApi: MovieApi) :
    FlowUseCase<Unit, List<MovieModel>>() {

    override fun execute(params: Unit): Flow<Result<List<MovieModel>>> = flow {
        val responseMovies = movieApi.getPopularMovies(DEF_FIRST_PAGE).results ?: emptyList()
